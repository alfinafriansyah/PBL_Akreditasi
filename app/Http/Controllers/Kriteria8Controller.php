...
class Kriteria8Controller extends Controller
{
    public function index()
    {
        ...
        $kriteria = KriteriaModel::where('kriteria_kode', 'KRT8')->first();
        
        return view('kriteria8.index', compact('breadcrumb', 'activeMenu', 'page', 'kriteria', 'status'));
    }
    ...
    public function list(Request $request)
    {
        ...
        return DataTables::of($kriterias)
            ...
            ->addColumn('aksi', function ($kriteria) { 
                $btn = '<button onclick="modalAction(\'' . url('/kriteria8/' . $kriteria->kriteria_id . '/detail') . '\')" class="btn btn-info btn-sm">Detail</button> ';
            ...
            if ($kriteria->status_id == 6) {
                ...
                $btn .= '<a href="' . url('/kriteria8/' . $kriteria->kriteria_id . '/edit') . '" class="btn btn-warning btn-sm">Edit</a> ';
                $btn .= '<button onclick="confirmDelete(\'' . url('/kriteria8/' . $kriteria->kriteria_id . '/delete') . '\', \'' . $kriteria->kriteria_id . '\')" class="btn btn-danger btn-sm">Hapus</button> ';
            }
                return $btn;
            })
    ...
    public function create()
    {
        $breadcrumb = (object) [
            ...
            'title' => 'Tambah Data Kriteria 8',
            'list' => 'Tambah Kriteria 8',
        ];
        ...
        return view('kriteria8.create', ['breadcrumb' => $breadcrumb, 'activeMenu' => $activeMenu, 'page' => $page]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($documentFields as $field => $folder) {
                ...
                if ($request->hasFile($field)) {
                    foreach ($request->file($field) as $file) {
                        $path = $file->store("public/documents/kriteria8/{$folder}");
            ...
            $penetapan = PenetapanModel::create([
                'kriteria_id' => 8, // kriteria_id is 8 for Kriteria 8
            ...
            return redirect('/kriteria8')->with('success', 'Data berhasil disimpan!');
    ...
    public function detail(string $id)
    {
        ...
        return view('kriteria8.detail', compact('kriteria'));
    }
    ...
    public function edit(string $id)
    {
        ...
        $breadcrumb = (object) [
            'title' => 'Edit Data Kriteria 8',
            'list' => 'Edit Kriteria 8',
        ];
        ...
        return view('kriteria8.edit', compact('kriteria', 'breadcrumb', 'activeMenu', 'page'));
    }
        ...
        try {
            ...
            foreach ($documentFields as $field => $relasi) {
                ...
                if ($request->hasFile($field)) {
                    ...
                    // Simpan file baru
                    foreach ($request->file($field) as $file) {
                        $path = $file->store("public/documents/kriteria8/{$relasi}");
                        $paths[] = str_replace('public/', 'storage/', $path);
                    }
                    $documentPaths[$field] = !empty($paths) ? json_encode($paths) : null;
                } else {
            ...
            return redirect('/kriteria8')->with('success', 'Data berhasil diupdate!');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect('/kriteria8')->with('success', 'Data berhasil dihapus!');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect('/kriteria8')->with('error', 'Gagal menghapus data: ' . $e->getMessage());
        }
    }
    ...
    public function indexEvaluasi()
    {
        ...
        return view('kriteria8.notifikasi', ['breadcrumb' => $breadcrumb, 'activeMenu' => $activeMenu, 'page' => $page, 'status' => $status]);
    }
