...
class Kriteria3Controller extends Controller
{
    public function index()
    {
        ...
        $kriteria = KriteriaModel::where('kriteria_kode', 'KRT3')->first();
        
        return view('kriteria3.index', compact('breadcrumb', 'activeMenu', 'page', 'kriteria', 'status'));
    }
    ...
    public function list(Request $request)
    {
        ...
        return DataTables::of($kriterias)
            ...
            ->addColumn('aksi', function ($kriteria) { 
                $btn = '<button onclick="modalAction(\'' . url('/kriteria3/' . $kriteria->kriteria_id . '/detail') . '\')" class="btn btn-info btn-sm">Detail</button> ';
            ...
            if ($kriteria->status_id == 6) {
                ...
                $btn .= '<a href="' . url('/kriteria3/' . $kriteria->kriteria_id . '/edit') . '" class="btn btn-warning btn-sm">Edit</a> ';
                $btn .= '<button onclick="confirmDelete(\'' . url('/kriteria3/' . $kriteria->kriteria_id . '/delete') . '\', \'' . $kriteria->kriteria_id . '\')" class="btn btn-danger btn-sm">Hapus</button> ';
            }
    ...
    {
        $breadcrumb = (object) [
            'title' => 'Tambah Data Kriteria 3',
            'list' => 'Tambah Kriteria 3',
        ];
        ...
        return view('kriteria3.create', ['breadcrumb' => $breadcrumb, 'activeMenu' => $activeMenu, 'page' => $page]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($documentFields as $field => $folder) {
                ...
                if ($request->hasFile($field)) {
                    foreach ($request->file($field) as $file) {
                        $path = $file->store("public/documents/kriteria3/{$folder}");
            ...
            $penetapan = PenetapanModel::create([
                'kriteria_id' => 3, // kriteria_id is 3 for Kriteria 3
            ...
            return redirect('/kriteria3')->with('success', 'Data berhasil disimpan!');
    ...
    public function detail(string $id)
    {
        ...
        return view('kriteria3.detail', compact('kriteria'));
    }
    ...
    public function edit(string $id)
    {
        ...
        $breadcrumb = (object) [
            'title' => 'Edit Data Kriteria 3',
            'list' => 'Edit Kriteria 3',
        ];
        ...
        return view('kriteria3.edit', compact('kriteria', 'breadcrumb', 'activeMenu', 'page'));
    }
        ...
        try {
            ...
            foreach ($documentFields as $field => $relasi) {
                ...
                if ($request->hasFile($field)) {
                    ...
                    // Simpan file baru
                    foreach ($request->file($field) as $file) {
                        $path = $file->store("public/documents/kriteria3/{$relasi}");
                        $paths[] = str_replace('public/', 'storage/', $path);
                    }
                    $documentPaths[$field] = !empty($paths) ? json_encode($paths) : null;
                } else {
            ...
            return redirect('/kriteria3')->with('success', 'Data berhasil diupdate!');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect('/kriteria3')->with('success', 'Data berhasil dihapus!');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect('/kriteria3')->with('error', 'Gagal menghapus data: ' . $e->getMessage());
        }
    }
    ...
    public function indexEvaluasi()
    {
        ...
        return view('kriteria3.notifikasi', ['breadcrumb' => $breadcrumb, 'activeMenu' => $activeMenu, 'page' => $page, 'status' => $status]);
    }
