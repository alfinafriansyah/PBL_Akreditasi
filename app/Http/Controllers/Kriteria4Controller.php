...
class Kriteria4Controller extends Controller
{
    ...
    {
        $breadcrumb = (object) [
            'title' => 'Tambah Data Kriteria 4',
            'list' => 'Tambah Kriteria 4',
        ];
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($documentFields as $field => $folder) {
                ...
                if ($request->hasFile($field)) {
                    foreach ($request->file($field) as $file) {
                        $path = $file->store("public/documents/kriteria4/{$folder}");
            ...
            $penetapan = PenetapanModel::create([
                'kriteria_id' => 4, // kriteria_id is 4 for Kriteria 4
    ...
    public function edit(string $id)
    {
        ...
        $breadcrumb = (object) [
            'title' => 'Edit Data Kriteria 4',
            'list' => 'Edit Kriteria 4',
        ];
        ...
        try {
            ...
            foreach ($documentFields as $field => $relasi) {
                ...
                if ($request->hasFile($field)) {
                    ...
                    // Simpan file baru
                    foreach ($request->file($field) as $file) {
                        $path = $file->store("public/documents/kriteria4/{$relasi}");
                        $paths[] = str_replace('public/', 'storage/', $path);
                    }
                    $documentPaths[$field] = !empty($paths) ? json_encode($paths) : null;
                } else {
            ...
            return redirect('/kriteria4')->with('success', 'Data berhasil diupdate!');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect('/kriteria4')->with('success', 'Data berhasil dihapus!');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect('/kriteria4')->with('error', 'Gagal menghapus data: ' . $e->getMessage());
        }
    }
