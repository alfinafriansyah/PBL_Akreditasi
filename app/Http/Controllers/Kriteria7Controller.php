...
class Kriteria7Controller extends Controller
{
    public function index()
    {
        ...
        $kriteria = KriteriaModel::where('kriteria_kode', 'KRT7')->first();
        
        return view('kriteria7.index', compact('breadcrumb', 'activeMenu', 'page', 'kriteria', 'status'));
    }
    ...
    {
        $breadcrumb = (object) [
            'title' => 'Tambah Data Kriteria 7',
            'list' => 'Tambah Kriteria 7',
        ];
        ...
        return view('kriteria7.create', ['breadcrumb' => $breadcrumb, 'activeMenu' => $activeMenu, 'page' => $page]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($documentFields as $field => $folder) {
                ...
                if ($request->hasFile($field)) {
                    foreach ($request->file($field) as $file) {
                        $path = $file->store("public/documents/kriteria7/{$folder}");
            ...
            $penetapan = PenetapanModel::create([
                'kriteria_id' => 7, // kriteria_id is 7 for Kriteria 7
            ...
            return redirect('/kriteria7')->with('success', 'Data berhasil disimpan!');
    ...
    public function detail(string $id)
    {
        ...
        return view('kriteria7.detail', compact('kriteria'));
    }
    ...
    public function edit(string $id)
    {
        ...
        $breadcrumb = (object) [
            'title' => 'Edit Data Kriteria 7',
            'list' => 'Edit Kriteria 7',
        ];
        ...
        return view('kriteria7.edit', compact('kriteria', 'breadcrumb', 'activeMenu', 'page'));
    }
        ...
        try {
            ...
            foreach ($documentFields as $field => $relasi) {
                ...
                if ($request->hasFile($field)) {
                    ...
                    // Simpan file baru
                    foreach ($request->file($field) as $file) {
                        $path = $file->store("public/documents/kriteria7/{$relasi}");
                        $paths[] = str_replace('public/', 'storage/', $path);
                    }
                    $documentPaths[$field] = !empty($paths) ? json_encode($paths) : null;
                } else {
            ...
            return redirect('/kriteria7')->with('success', 'Data berhasil diupdate!');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect('/kriteria7')->with('success', 'Data berhasil dihapus!');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect('/kriteria7')->with('error', 'Gagal menghapus data: ' . $e->getMessage());
        }
    }
